...
- **What it does:** Serves as a strictly read-only coordinator that pulls together the findings of all other system agents into a single, deduplicated, prioritized action list for administrators:
  ...
  - **Interactive Action Center & Bulk Review:** Accessible at `/admin/agents` (and `/admin/action-center`), featuring direct deep links to existing resolution pages, multi-select checkboxes for batch operations ("Generate Drafts for Selected", "Dismiss Selected"), and a session-based **"Dismiss from this view"** toggle that declutters the admin's workspace without touching or deleting any database records.

### 8. Admin Assistant Agent (`admin_assistant.py`)
- **What it does:** Provides an interactive, chat-style guidance panel on the Action Center page (`/admin/agents#action-center`) where administrators can ask plain-language questions (e.g., *"What should I do next?"*, *"Did I make any mistakes?"*, *"What's wrong with the Science & Nature category?"*).
- **Strictly Grounded in Real Telemetry:** The assistant answers using ONLY real data already gathered by the Orchestrator Agent and other system agents (pending suggestions, catalog integrity issues, platform/cohort health metrics, recent audit log entries, and recent compliance incidents). The assistant is strictly prohibited from inventing or hallucinating claims, numbers, or recommendations.
